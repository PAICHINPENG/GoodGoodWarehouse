...
<body>
    ...
    <?php include 'topnavbar.php'?>
	 ...
	 <div id="content" style="position:absolute; top:110px; bottom:60px; left:0px; right:0px; overflow:auto;background-image:url( 'https://pic50.photophoto.cn/20190320/1190120702716590_b.jpg' );">
        <input type="button" value="回倉庫" onclick="location.href='warehousePC.php'" style="
position:fixed; left:50px; bottom:70px;background-color: orange;">

	 	<!-- Button to Open the Modal -->
<button type="button" style="
position:absolute; right:50px; bottom:50px;background-color: orange;
  ...
  if($query->num_rows >= 0){
      while($row = $query->fetch_assoc()){
       ...
       <div id="r">
		...
		<form action="goodsPC.php" method="post">
              <input type="hidden" name="wnnn" value="<?=$wname?>">
              <input type="hidden" name="bnnn" value= "<?=$bname?>">
              <button type="submit" style="background-color: transparent; outline-color: yellow; border-radius: 50%;"name="boximg" value="<?= $bid?>"><img style="border-radius: 50%;" height="200px" width="200px" src="<?= $imageURL;?>"></button>
          </form>
          <button data-id='<?php echo $row['boxid']?>' class="iiuserinfo btn btn-success" style="margin-left: 8% ;font-size: 0.8em;"><?= $bname?></button></td>
<div class="modal fade" id="bempModal" role="dialog">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h4 class="modal-title">箱子小檔案</h4>
		...
		<form action="delete.php" method="post">
          ...
          <button type="submit" class="btn btn-danger" name="b" value="<?= $bid?>" style="margin-top: -32%; margin-left: 25%;">刪除</button>
    </form>
...
<div class="modal" id="myModal7">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <!-- Modal Header -->
      <div class="modal-header">
        <h4 class="modal-title">新增箱子資訊</h4>
      ...
      <img id="pre" width="50px" height="50px"><br>
      <input type="text"  name="boxname" placeholder="箱子的名稱"><font size="6"></font>
      <div><br>
        <textarea id="text1" name="image_text"placeholder="對箱子的描述"></textarea>
      </div><br>
      <select name="t">
